%% mobility_cell_eval_mc.m
% Cellular connectivity under mobility with Doppler + HO/RLF penalties + Monte Carlo
clear; close all; clc;
...
for modeIdx=1:numel(deployments)
    ...
    for sIdx=1:numel(speeds)
        ...
        for mc=1:N_mc
            ...
            fadingCorr=zeros(N_bs,N_t); shadowCorr=zeros(N_bs,N_t);
            for b=1:N_bs
                w=(randn(N_t,1)+1j*randn(N_t,1))/sqrt(2);
                if fc_cut/(fs/2)<0.99, h_lp=fir1(L-1,min(0.99,fc_cut/(fs/2))); hf=filter(h_lp,1,w); else hf=w; end
            ...
            for tt=1:N_t
                ...
                shadow = shadowCorr(:,tt); fading_dB=20*log10(fadingCorr(:,tt)+1e-9);
...
%% Plotting (same as before, SINR, CDF, bar charts)
%% ------------------ Plotting ------------------
