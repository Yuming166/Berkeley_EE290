...
% Cellular connectivity under mobility with Doppler + HO/RLF penalties + Monte Carlo + 2x2 MIMO
clear; close all; clc;
...
%% MIMO parameters
Nt = 2; % transmit antennas per BS
Nr = 2; % receive antennas per UE
...
for modeIdx=1:numel(deployments)
    ...
    for sIdx=1:numel(speeds)
        ...
        for mc=1:N_mc
            ...
            fadingCorr = zeros(N_bs,Nr,Nt,N_t);
            shadowCorr=zeros(N_bs,N_t);
            
            for b=1:N_bs
                sigma_s = sigma_shadow(char(bs_type(b)));
                % Shadowing
                w2 = randn(N_t,1);
                if fc_cut/(fs/2)<0.99
                ...
                hs = hs/std(hs);
                shadowCorr(b,:) = sigma_s*hs(:).';
                
                % Fading for each antenna
                for r=1:Nr
                    for t=1:Nt
            ...
            for tt=1:N_t
                d = sqrt((ue_pos(tt,1)-pos_bs(:,1)).^2 + (ue_pos(tt,2)-pos_bs(:,2)).^2); d=max(d,d_min); PL=PL_func(d);
                
                % --- Compute MIMO received power ---
                Rx_linear = zeros(N_bs,1);
                for b=1:N_bs
                    H = reshape(fadingCorr(b,:,:,tt), [Nr, Nt]);  % 保证Nr×Nt
                    Rx_linear(b) = sum(sum(abs(H).^2));           % MRC combining -> scalar
                    Rx_linear(b) = Rx_linear(b) * 10^(shadowCorr(b,tt)/10); 
                    Rx_linear(b) = Rx_linear(b) * 10^((Pt_bs(b)-PL(b))/10);
                end
                Rx_dBm = 10*log10(Rx_linear);
                RSRP_dBm(tt,:) = Rx_dBm';
                ...
                if tt>1 && servIdx~=prevCell
                    ho_count = ho_count+1;
                    throughput_Mbps(tt) = throughput_Mbps(tt) * (1-HO_delay/dt); % penalty
                end
                prevCell = servIdx;
                ...
                % Throughput (multiply by spatial streams Nt)
                throughput_Mbps(tt) = Nt*BW*log2(1+SINR_lin)/1e6;
        ...
        results(modeIdx).speed(sIdx).handover_count=mean(ho_count_mc); results(modeIdx).speed(sIdx).rlf_count=mean(rlf_count_mc);
...
for sIdx = 1:numel(speeds)
    ...
    title(sprintf('MIMO Throughput CDF at %d mph', speeds_mph(sIdx)));
    hold off;
end

% 3) Handover, RLF, Avg SINR and Avg Throughput bar charts
...
figure('Name','MIMO Handover count');
bar(ho_matrix');
set(gca,'XTickLabel',deployments);
legend(string(speeds_mph)+" mph",'Location','best');
ylabel('Handover count'); grid on; title('MIMO Handover count comparison');

figure('Name','MIMO RLF count');
bar(rlf_matrix');
set(gca,'XTickLabel',deployments);
legend(string(speeds_mph)+" mph",'Location','best');
ylabel('RLF count'); grid on; title('MIMO RLF count comparison');

figure('Name','MIMO Avg SINR');
bar(avg_sinr');
set(gca,'XTickLabel',deployments);
legend(string(speeds_mph)+" mph",'Location','best');
ylabel('Average SINR (dB)'); grid on; title('MIMO Average SINR');

figure('Name','MIMO Avg Throughput');
bar(avg_thr');
set(gca,'XTickLabel',deployments);
legend(string(speeds_mph)+" mph",'Location','best');
ylabel('Average Throughput (Mbps)'); grid on; title('MIMO Average Throughput');